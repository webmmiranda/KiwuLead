...
import { CurrentUser, Contact, TeamMember, Task, DistributionSettings, AutomationRule, LeadStatus, Source, Product, EmailTemplate, CompanyProfile, Notification, AiConfig, Appointment, FeaturesConfig } from './types';
import { DEFAULT_AUTOMATIONS } from './constants';
import { Menu, Loader2, Bell, X, Check, LayoutDashboard, MessageSquare, Kanban, Users, Package, Mail, Calendar as CalendarIcon, CheckSquare } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const bottomNavItems = [
    ...
    if (item.id === 'mail' && !features.enableEmail) return false;
    if (item.id === 'inbox' && !features.enableWhatsApp) return false;
    return true;
  });
  ...
  useEffect(() => {
    const initSystem = async () => {
      ...
      try {
        ...
        if (status && typeof status.installed !== 'undefined' && !status.installed) {
  ...
  const loadData = async () => {
    try {
      ...
      if (fetchedSettings.features) setFeatures(fetchedSettings.features);
  ...
  const renderContent = () => {
    ...
    switch (activeTab) {
      ...
      case 'products':
        return <Products products={products} setProducts={setProducts} currentUser={currentUser} onNotify={addNotification} aiConfig={aiConfig} />;
      case 'inbox':
        return (
          <Inbox
            currentUser={currentUser}
      ...
      case 'reports':
        return <Reports currentUser={currentUser} contacts={contacts} team={team} tasks={tasks} companyCurrency={companyProfile.currency} features={features} />;
      case 'support':
      ...
      case 'mail':
        return <MailClient currentUser={currentUser} onNotify={addNotification} aiConfig={aiConfig} />;
      case 'user-profile':
        ...
        <div className="flex h-screen bg-slate-50 font-sans text-slate-900 overflow-hidden">
          ...
          <div className="lg:hidden fixed top-0 left-0 right-0 z-30 bg-slate-900 flex items-center px-4 justify-between shadow-md pt-safe h-[calc(4rem+env(safe-area-inset-top))]">
            <div className="flex items-center gap-3">
              <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} title="Menú" aria-label="Abrir menú lateral" className="text-white p-1">
                <Menu size={24} />
              </button>
              <span className="text-lg font-bold text-white tracking-tight">{companyProfile.name}</span>
            </div>
            <div className="flex items-center gap-4">
              <button onClick={() => setIsNotifOpen(!isNotifOpen)} title="Notificaciones" aria-label="Abrir notificaciones" className="relative text-slate-300">
                <Bell size={24} />
          ...
          <main className={`flex-1 overflow-auto relative pt-[calc(4rem+env(safe-area-inset-top))] w-full flex flex-col pb-[calc(6rem+env(safe-area-inset-bottom))] lg:pb-0 transition-all duration-300 ${isDemoMode ? 'lg:pt-10' : 'lg:pt-0'}`}>
            {originalUser && (
