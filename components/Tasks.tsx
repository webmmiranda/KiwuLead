...
export const Tasks: React.FC<TasksProps> = ({ currentUser, tasks, setTasks, appointments, contacts }) => {
    ...
    return (
        <div className="h-full flex flex-col bg-slate-50 overflow-hidden">
            {/* Header */}
            <div className="bg-white border-b border-slate-200 px-4 md:px-6 py-4 flex flex-row justify-between items-center gap-4">
                <div>
                    <h1 className="text-xl md:text-2xl font-bold text-slate-900 flex items-center gap-2">
                        <CheckSquare className="text-blue-600 md:w-6 md:h-6" size={20} /> Mis Tareas
                    </h1>
                    <p className="text-slate-500 text-xs md:text-sm hidden xs:block">Gestiona tus actividades y recordatorios.</p>
                </div>
                <button
                    onClick={() => {
                        setEditingTask(undefined);
                    ...
                    className="bg-blue-600 hover:bg-blue-700 text-white px-3 md:px-4 py-2 rounded-lg flex items-center gap-2 font-medium transition-colors whitespace-nowrap"
                >
                    <Plus size={18} /> <span className="hidden sm:inline">Nueva Tarea</span><span className="sm:hidden">Nuevo</span>
                </button>
            </div>
