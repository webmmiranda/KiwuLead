...
import { Contact, Message, Product, EmailTemplate, CurrentUser, Task } from '../types';
...
export const Inbox: React.FC<InboxProps> = ({ currentUser, contacts, setContacts, tasks, setTasks, onMessageSent, products, templates, initialContactId, onNotify }) => {
  ...
  const handleSendMessage = () => {
    ...
    const newMessage: Message = {
      ...
      sender: inputMode === 'note' ? 'agent' : 'agent',
        ...
        <div className="p-4 border-b border-slate-200">
          <div className="relative">
            ...
            <input 
              ...
              className="w-full pl-10 pr-4 py-2 bg-white border border-slate-300 text-slate-900 placeholder-slate-500 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 outline-none"
            />
          </div>
        </div>
        <div className="flex-1 overflow-y-auto">
          {displayedContacts.map(contact => {
             ...
             return (
                ...
                className={`p-4 border-b border-slate-100 cursor-pointer hover:bg-slate-50 transition-colors ${selectedContactId === contact.id ? 'bg-indigo-50 border-l-4 border-l-indigo-600' : ''}`}
                >
                <div className="flex justify-between mb-1">
                    <h4 className={`text-sm ${needsReply ? 'font-bold text-slate-900' : 'font-semibold text-slate-800'}`}>{contact.name}</h4>
                    <span className={`text-xs ${needsReply ? 'text-indigo-600 font-bold' : 'text-slate-400'}`}>
      ...
      <div className="flex-1 flex flex-col bg-slate-100">
        ...
        <div className="bg-white px-6 py-3 border-b border-slate-200 flex justify-between items-center shadow-sm z-10">
          ...
          <div className="flex space-x-2">
            <button className="p-2 text-slate-500 hover:text-indigo-600 hover:bg-slate-100 rounded-full transition-colors"><Phone size={20} /></button>
            <button className="p-2 text-slate-500 hover:text-indigo-600 hover:bg-slate-100 rounded-full transition-colors"><Video size={20} /></button>
          </div>
        </div>
          ...
          {activeContact.history.map((msg) => {
            ...
            return (
                ...
                <div className={`max-w-[70%] rounded-2xl px-4 py-3 shadow-sm ${
                    msg.sender === 'agent' 
                    ? (msg.channel === 'email' ? 'bg-blue-600 text-white rounded-br-none' : 'bg-indigo-600 text-white rounded-br-none') 
                    ...
                    <p className={`text-[10px] mt-1 text-right ${msg.sender === 'agent' ? 'text-indigo-200' : 'text-slate-400'}`}>
                    {msg.timestamp}
                    </p>
                </div>
                </div>
        ...
        <div className={`p-4 border-t border-slate-200 transition-colors duration-300 ${inputMode === 'note' ? 'bg-amber-50' : 'bg-white'}`}>
          ...
          {channel === 'email' && inputMode === 'message' && (
              <input 
                  ...
                  className="w-full mb-2 px-3 py-2 bg-white border border-slate-300 rounded text-slate-900 text-sm focus:outline-none focus:border-indigo-500"
              />
          )}
          ...
          <div className="relative flex items-end gap-2">
             {inputMode === 'message' && (
                 <button 
                    ...
                    disabled={isTyping}
                    className="p-3 text-indigo-600 bg-indigo-50 hover:bg-indigo-100 rounded-lg transition-colors"
            ...
            <textarea
              ...
              className={`flex-1 border text-slate-900 placeholder-slate-500 rounded-lg focus:ring-2 focus:border-transparent resize-none p-3 text-sm max-h-32 min-h-[80px] ${inputMode === 'note' ? 'bg-white border-amber-300 focus:ring-amber-500' : 'bg-white border-slate-300 focus:ring-indigo-500'}`}
              onKeyDown={(e) => {
